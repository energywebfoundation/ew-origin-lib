...
export class EnergyDB extends GeneralFunctions {
    ...
    async setPurchasePrice(_entityId: number, _price: number, txParams?: SpecialTx) {
        let transactionParams;

        const txData = await this.web3Contract.methods
            .setPurchasePrice(_entityId, _price)
            .encodeABI();
    ...
    async publishForSale(_entityId: number, _price: number, _tokenAddress: string, txParams?: SpecialTx) {
        ...
        const txData = await this.web3Contract.methods.publishForSale(
            _entityId, _price, _tokenAddress
        ).encodeABI();
    ...
    async publishForSaleFiat(_entityId: number, _price: number, _currency: Currency, txParams?: SpecialTx) {
        let transactionParams;

        const txData = await this.web3Contract.methods.publishForSaleFiat(
            _entityId, _price, _currency
        ).encodeABI();
        ...
        if (txParams) {
            ...
            if (!txParams.gas) {
                try {
                    gas = await this.web3Contract.methods
                        .publishForSaleFiat(_entityId, _price, _currency)
                        .estimateGas({
                            from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]
                        });
                } catch (ex) {
    ...
    async setOffChainCurrency(_entityId: number, _currency: Currency, txParams?: SpecialTx) {
        let transactionParams;

        const txData = await this.web3Contract.methods
            .setOffChainCurrency(_entityId, _currency)
            .encodeABI();
    ...
    async getPurchasePrice(_entityId: number, txParams?: SpecialTx) {
        return await this.web3Contract.methods
            .getPurchasePrice(_entityId)
            .call(txParams);
    }
    ...
    async getOffChainCurrency(_entityId: number, txParams?: SpecialTx) {
        return await this.web3Contract.methods.getOffChainCurrency(_entityId).call(txParams);
    }
}
